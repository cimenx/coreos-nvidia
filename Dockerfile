...
RUN wget -P /opt/nvidia http://us.download.nvidia.com/XFree86/Linux-x86_64/367.27/NVIDIA-Linux-x86_64-367.27.run
...
WORKDIR /opt/nvidia
RUN echo "./NVIDIA-Linux-x86_64-367.27.run -q -a -n -s --kernel-source-path=/usr/src/kernels/linux/ && modprobe nvidia" >> install_kernel_module && \
    chmod +x NVIDIA-Linux-x86_64-367.27.run install_kernel_module
